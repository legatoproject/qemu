/*
 *  Qualcomm chipset 9x28 emulation: msm9x28.c
 *
...
static void msm9x28_smd_smsm_init(const MSM_BoardInfo *b,MemoryRegion *memory,qemu_irq *pic)
{
hwaddr base;
int irq;
uint32_t size;
    ...
    msm9x28_smd_init(memory, base, pic[irq], size);
    ...
    msm9x28_smem_targ_info_init(memory, base, pic[irq], size);

#if 0
    base = b->memmap[vSMEM_AUX1].base;
    irq = b->irqmap[vSMEM_AUX1];
    size = b->memmap[vSMEM_AUX1].size;
    msm9x28_smem_aux1_init(memory, base, pic[irq], size);
#endif
 }
...
static void msm9x28_flash_init(const MSM_BoardInfo *b)
{
...
/******************************************
* Board init.
* initialize MSM9x28 machine with cortex-a15 instead of cortex-a5, which is no supported yet on QEMU
******************************************/
...
static void msm9x28_net_init(const MSM_BoardInfo *b, qemu_irq *pic)
{
...
MSM_BoardInfo *b=NULL;
    ...
    msm9x28_net_init(b,pic_9x28);

    // SMD & SMSM initialization
    msm9x28_smd_smsm_init(b,sysmem,pic_9x28);

    msm9x28_flash_init(b);
