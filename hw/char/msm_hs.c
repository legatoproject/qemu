/*
 *  Qualcomm MSM HS emulation: msm_hs.c
 *
...
#define	UARTDM_IMR_REG  0xb0	// WO
#define	UARTDM_SR_REG   0xa4	// RO
#define	UARTDM_CR_REG   0xa8
#define	UARTDM_CSR_REG  0xa0
#define	UARTDM_IPR_REG  0x18
#define	UARTDM_ISR_REG  0xb4	// RO
#define	UARTDM_RX_TOTAL_SNAP_REG 0xbc
#define	UARTDM_RFWR_REG 0x20
#define	UARTDM_TFWR_REG 0x1c
#define	UARTDM_RF_REG   0x140
#define	UARTDM_TF_REG   0x100
#define	UARTDM_MISR_REG 0xac	// RO
...
#define UART_DM_RX_TRANS_CTRL 0xcc
#define UART_DM_BCR     0xc8

#define UARTDM_VERSION_11_13	0
#define UARTDM_VERSION_14	1

#define TYPE_MSM_HSL "msm_hs"
...
void msm_hs_set_baudrate(MsmHslCtx* s);
void msm_hs_update_termio_params(MsmHslCtx* s);
void msm_hs_fifo_tx(MsmHslCtx* s,int value);
void msm_hs_realize_core(MsmHslCtx *s, Error **errp);
MsmHslCtx* msm_hs_init(MemoryRegion *,hwaddr,qemu_irq, int,CharDriverState *);
...
static void msm_hs_uart_update_t_status(MsmHslCtx *s)
{
uint32_t flags;
...
static void msm_hs_uart_update_r_status(MsmHslCtx *s)
{
uint32_t flags;
...
static void msm_hs_read_rx_fifo(MsmHslCtx *s, uint32_t *c)
{
...
static void msm_hs_write_rx_fifo(void *opaque, const uint8_t *buf, int size)
{
MsmHslCtx *s = (MsmHslCtx *)opaque;
...
static gboolean msm_hs_uart_xmit(GIOChannel *chan, GIOCondition cond,void *opaque)
{
MsmHslCtx *s = opaque;
int ret;
    ...
    if (s->tx_count)
    {
        int r = qemu_chr_fe_add_watch(s->chr, G_IO_OUT|G_IO_HUP, msm_hs_uart_xmit, s);
        assert(r);
    }

    msm_hs_uart_update_t_status(s);
...
char* pchar = (char*)&value;
    ...
    msm_hs_uart_xmit(NULL, G_IO_OUT, s);
}

static void msm_hs_reset_rx(MsmHslCtx *s)
{
    s->rx_wpos = 0;
    s->rx_count = 0;
    if (s->chr)
...
static void msm_hs_reset_tx(MsmHslCtx *s)
{
    s->tx_count = 0;
}

static void msm_hs_start_break(MsmHslCtx *s)
{
int break_enabled = 1;
...
static void msm_hs_update_ctrl(MsmHslCtx *s)
{
    if (s->cr & RESET_TX)
    {
        msm_hs_reset_tx(s);
...
static void msm_hs_write(void *opaque, hwaddr offset, uint64_t value, unsigned size)
{
MsmHslCtx* s = (MsmHslCtx *)opaque;
    ...
    case UARTDM_CR_REG:
	s->cr |= value;
	msm_hs_update_ctrl(s);
	break;
    case UARTDM_CSR_REG:
	s->csr = value;
	msm_hs_set_baudrate(s);
	break;
    case UARTDM_IPR_REG:
	s->ipr = value;
	break;
	...
	break;
    default:
 	qemu_log_mask(LOG_GUEST_ERROR,"msm_hs_write: Bad offset %x\n",(int)offset);
	break;
    }
}

static uint64_t msm_hs_read(void *opaque, hwaddr offset, unsigned size)
{
MsmHslCtx *s = (MsmHslCtx *)opaque;
uint32_t ret = 0;
	...
	break;
    default:
 	qemu_log_mask(LOG_GUEST_ERROR,"msm_hs_read: Bad offset %x\n",(int)offset);
	break;
    }
...
static int msm_hs_can_receive(void *opaque)
{
MsmHslCtx *s = (MsmHslCtx *)opaque;
int ret;
...
static void msm_hs_receive(void *opaque, const uint8_t *buf, int size)
{
    //fprintf(stderr,"MSM HSL UART :: Data received\n");
    msm_hs_write_rx_fifo(opaque, buf, size);
}

static void msm_hs_event(void *opaque, int event)
{
...
static const VMStateDescription vmstate_msm_hs = {
    .name = "msm_hs",
    .version_id = 1,
...
static const MemoryRegionOps msm_hs_mm_ops = {
   .read = msm_hs_read,
   .write = msm_hs_write,
...
static void msm_hs_reset(void *opaque)
{
MsmHslCtx *s = (MsmHslCtx *)opaque;
...
void msm_hs_realize_core(MsmHslCtx *s, Error **errp)
{
    ...
    qemu_register_reset(msm_hs_reset, s);
    qemu_chr_add_handlers(s->chr, msm_hs_can_receive, msm_hs_receive, msm_hs_event, s);
    msm_hs_reset(s);
}
...
printf( "msm_hs_init\n" );
    ...
    msm_hs_realize_core(s, &err);
    if (err != NULL) {
        error_report("%s", error_get_pretty(err));
        error_free(err);
        exit(1);
    }
    vmstate_register(NULL, base, &vmstate_msm_hs, s);

    memory_region_init_io(&s->iomem, NULL, &msm_hs_mm_ops, s, "msm_hs", 0x1000);
